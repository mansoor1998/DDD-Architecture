...
async def test_register_user_success(user_service, mock_user_repo, mock_email_sender):
    ...
    with patch(
        "app.domain.services.user_service.get_password_hash", return_value="hashed_pwd"
    ):
        ...
        mock_email_sender.send_verification_email.assert_called_once_with(
            email, user.verification_token
        )
...
@pytest.mark.asyncio
async def test_authenticate_user_not_found_throws_invalid_credentials(
...
@pytest.mark.asyncio
async def test_authenticate_user_wrong_password_throws_invalid_credentials(
    user_service, mock_user_repo
):
