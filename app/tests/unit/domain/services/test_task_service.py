...
async def test_get_task_by_id_found(task_service, mock_task_repo):
    ...
    task_id = uuid4()
    expected_task = Task(
        id=task_id, user_id=user_id, title="Found Task", description="Existent"
    )
...
async def test_get_task_by_id_access_denied(task_service, mock_task_repo):
    ...
    task = Task(
        id=task_id, user_id=owner_id, title="Private Task", description="Secret"
    )
...
async def test_update_task_success(task_service, mock_task_repo):
    ...
    task_id = uuid4()
    existing_task = Task(
        id=task_id, user_id=user_id, title="Old Title", description="Old Desc"
    )
    update_data = {"title": "New Title"}
    updated_task = Task(
        id=task_id, user_id=user_id, title="New Title", description="Old Desc"
    )
...
async def test_delete_task_success(task_service, mock_task_repo):
    ...
    task_id = uuid4()
    existing_task = Task(
        id=task_id, user_id=user_id, title="To Delete", description="Bye"
    )
