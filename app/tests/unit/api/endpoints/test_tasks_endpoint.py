...
from app.api.endpoints.tasks import create_task, get_tasks, get_task, update_task, delete_task
...
async def test_create_task_success(mocker, mock_user):
    ...
    res = await create_task(task_in=mock_task_in, current_user=mock_user, task_service=mock_svc)
...
async def test_create_task_limit_reached(mocker, mock_user):
    ...
    with pytest.raises(HTTPException) as exc:
        await create_task(task_in=mocker.Mock(), current_user=mock_user, task_service=mock_svc)
...
async def test_update_task_success(mocker, mock_user, task_id):
    ...
    res = await update_task(task_id=task_id, task_in=mock_task_in, current_user=mock_user, task_service=mock_svc)
    
    assert res["title"] == "Updated Task"
    mock_svc.update_task.assert_called_once_with(task_id, {"title": "Updated Task"}, mock_user.id)
...
async def test_update_task_access_denied(mocker, mock_user, task_id):
    ...
    with pytest.raises(HTTPException) as exc:
        await update_task(task_id=task_id, task_in=mocker.Mock(), current_user=mock_user, task_service=mock_svc)
...
async def test_delete_task_success(mocker, mock_user, task_id):
    ...
    res = await delete_task(task_id=task_id, current_user=mock_user, task_service=mock_svc)
...
async def test_delete_task_not_found(mocker, mock_user, task_id):
    ...
    with pytest.raises(HTTPException) as exc:
        await delete_task(task_id=task_id, current_user=mock_user, task_service=mock_svc)
