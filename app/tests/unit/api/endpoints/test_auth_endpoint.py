...
from app.api.endpoints.auth import register, resend_verification_email, verify_email, login_access_token
...
async def test_register_success(mocker):
    ...
    mock_svc.register_user.return_value = mocker.Mock(email="test@test.com", is_active=True)
    mocker.patch("app.api.endpoints.auth.create_access_token", return_value="fake-token")
...
async def test_resend_verification_success(mocker):
    ...
    res = await resend_verification_email(user_service=mock_svc, current_user=inactive_user)
...
async def test_resend_verification_already_active(mocker):
    ...
    with pytest.raises(HTTPException) as exc:
        await resend_verification_email(user_service=mocker.AsyncMock(), current_user=active_user)
...
async def test_login_success(mocker):
    mock_svc = mocker.AsyncMock()
    mock_svc.authenticate_user.return_value = mocker.Mock(email="test@test.com", is_active=True)
    mocker.patch("app.api.endpoints.auth.create_access_token", return_value="fake-token")
    mocker.patch("app.core.config.settings.ACCESS_TOKEN_EXPIRE_MINUTES", 30)

    form_data = OAuth2PasswordRequestForm(username="test@test.com", password="password", scope="")
...
async def test_login_invalid_credentials(mocker):
    ...
    form_data = OAuth2PasswordRequestForm(username="test@test.com", password="wrong", scope="")
    with pytest.raises(HTTPException) as exc:
